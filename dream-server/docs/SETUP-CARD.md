...
`scripts/generate-setup-card.py` produces printable 4x6 setup cards for fulfillment: drop one in the box with each Dream Server unit so the recipient can scan two QR codes without typing anything. The default card opens the first-run wizard; `--mode factory-owner` prints the owner-card handoff that opens Hermes.
...
2. **Two QR codes side by side:**
   ...
   - **Right - OPEN SETUP or OPEN HERMES.** In default setup mode this is a setup URL such as `http://192.168.7.1/setup`. In factory-owner mode this is the owner magic-link URL generated by Setup / Owner.
...
| Flag | Required | Notes |
|---|---|---|
| `--mode` | no | `setup` by default; use `factory-owner` for Wi-Fi + owner Hermes QR cards. |
